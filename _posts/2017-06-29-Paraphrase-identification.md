...
This Neural Network has been trained several times with different modifications in its architecture and inputs. For this set of trainings, questions has been taken has inputs after removing (or not) stopwords, the architecture has been modified adding  more layers (including [Dropout](https://www.cs.toronto.edu/~hinton/absps/JMLRdropout.pdf) and [Batch Normalization](https://gab41.lab41.org/batch-normalization-what-the-hey-d480039a9e3b)) or increasing their size, and distance features are taken as an input to concatenate before (or not) been processed by a MLP layer. All of these models have achieved an **accuracy** between 88% and **91%** with the full labeled dataset.
